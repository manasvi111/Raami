<?php
session_start();
include '../universal/db.php'; // Include database connection
...
// Fetch total tasks and completed tasks for the progress tracker
...
<head>
    ...
    <link rel="stylesheet" href="../user/users.css?v=5.0"> 
</head>
<body>
    <!-- Header Section -->
    <header id="custom-header" class="text-white p-3 text-center">
        <h2>Welcome, <?php echo htmlspecialchars($user_name); ?></h2>
        <a href="../universal/logout.php" class="btn btn-light btn-sm">Logout</a>
    </header>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#addTaskButton').click(function() {
                ...
                if (taskName) {
                    $.post('../user/add_task.php', { task_name: taskName }, function(data) {
            ...
            $(document).on('change', '.task-checkbox', function() {
                ...
                $.post('update_task.php', { task_id: taskId, is_completed: isCompleted }, function(data) {
                    if (data.status === 'success') {
                        ...
                        updateProgressTracker(); // Update the progress tracker
                    } else {
            ...
            $(document).on('click', '.edit-task-btn', function() {
                ...
                if (newTaskName) {
                    $.post('../user/edit_task.php', { task_id: taskId, task_name: newTaskName }, function(data) {
                        if (data.status === 'success') {
            ...
            function updateProgressTracker() {
                $.get('../user/get_progress.php', function(data) {
                    if (data.status === 'success') {
                        const progress = data.progress_percentage;
                        $('.progress-circle span').text(Math.round(progress) + '%');
                    }
                }, 'json');
            }
        });
